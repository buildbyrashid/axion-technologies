...
export default function RichTextEditor({ content, onChange, placeholder = 'Start writing...' }: RichTextEditorProps) {
  ...
  const ToolbarButton = ({ onClick, active, children, title }: any) => (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-[#0D95F0]/10 text-[#0D95F0]' : 'hover:bg-slate-100 hover:text-slate-600'
      )}
    >
      {children}
    </button>
  ...
  return (
    <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white focus-within:border-[#0D95F0]/50 focus-within:ring-4 focus-within:ring-[#0D95F0]/5 transition-all">
      {/* Toolbar */}
