      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative" ref={notifRef}>
          <button 
            ...
            className="relative w-11 h-11 rounded-full bg-white/40 border border-black/5 flex items-center justify-center text-slate-400 hover:text-[#0D95F0] hover:bg-white hover:shadow-2xl hover:shadow-black/5 transition-all duration-500"
          >
            <Bell size={18} />
            {inquiries.length > 0 && (
          ...
          {showNotifications && (
            <div className="absolute top-full mt-2 right-0 w-80 bg-white rounded-2xl shadow-2xl shadow-black/10 border border-black/5 overflow-hidden z-50">
              <div className="p-4 border-b border-black/5 flex items-center justify-between bg-slate-50/50">
                <span className="text-xs font-black uppercase tracking-widest text-[#0A1628]">Notifications</span>
                {inquiries.length > 0 && (
                ...
                {inquiries.length === 0 ? (
                  ...
                  inquiries.map(inq => (
                    <div 
                      ...
                      className="p-4 border-b border-black/5 hover:bg-slate-50 transition-colors cursor-pointer group flex gap-3"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-black text-[#0A1628] truncate pr-2">{inq.name}</span>
                          <span className="text-[10px] text-slate-400 shrink-0">{new Date(inq.created_at).toLocaleDateString()}</span>
                        </div>
        ...
        {/* User Profile */}
        <Link href="/admin/settings" className="flex items-center gap-3 pl-1 pr-5 py-1.5 rounded-full hover:bg-white hover:shadow-2xl hover:shadow-black/5 transition-all duration-500 group bg-white/40 border border-black/5">
