    ...
    <div className="space-y-12 pb-24 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
        <div className="space-y-4">
           ...
           <h1 className="text-5xl font-extrabold text-[#0A1628] dark:text-white tracking-tighter leading-tight">Products Directory</h1>
           <p className="text-slate-500 dark:text-slate-400 dark:text-slate-300 text-lg font-medium max-w-2xl leading-relaxed">Create, edit, and organize the hardware products showcasing our 20+ years of industry experience.</p>
        </div>
      ...
      {/* Control Console */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 bg-white/40 backdrop-blur-3xl p-6 rounded-[1.75rem] border border-black/5 dark:border-white/10 shadow-sm">
         <div className="relative flex-1 max-w-xl group">
            ...
            <input 
               ...
               className="relative w-full pl-16 pr-8 py-5 rounded-[2rem] border border-black/5 dark:border-white/10 bg-white dark:bg-slate-900 shadow-sm focus:shadow-2xl focus:shadow-[#0D95F0]/10 outline-none transition-all text-sm font-black tracking-tight placeholder:text-slate-300"
            />
         </div>
         ...
         <div className="flex items-center gap-4">
            <div className="relative flex items-center gap-3">
               ...
               <select 
                  ...
                  className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-[2rem] pl-14 pr-12 py-5 text-[10px] sm:text-xs font-black text-[#0A1628] dark:text-white uppercase tracking-widest focus:ring-8 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none transition-all cursor-pointer w-full max-w-xs appearance-none shadow-sm hover:shadow-lg truncate"
               >
                  <option value="all">All Categories</option>
                  {categories.filter(c => !c.parent_id).map(mainCat => (
                     <optgroup key={mainCat.id} label={mainCat.name} className="font-black text-[#0A1628] dark:text-white bg-slate-50 dark:bg-slate-800 py-2">
                        <option value={mainCat.id} className="font-bold text-slate-700 bg-white dark:bg-slate-900 py-1">All {mainCat.name}</option>
                        {categories.filter(c => c.parent_id === mainCat.id).map(subCat => (
                           <option key={subCat.id} value={subCat.id} className="font-medium text-slate-500 dark:text-slate-400 dark:text-slate-300 bg-white dark:bg-slate-900 py-1 ml-4">&nbsp;&nbsp;— {subCat.name}</option>
                        ))}
                     </optgroup>
                  ))}
               </select>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-[2rem] overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="py-32 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 dark:text-slate-300 font-bold tracking-tight text-xl">No matching products found.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 text-slate-400 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredProducts.map((product) => (
                  <tr 
                    key={product.id}
                    className="hover:bg-slate-50/50 dark:bg-slate-800/50 transition-colors duration-200 group"
                  >
                    {/* Thumbnail Column */}
                    ...
                    <td className="py-6 px-6">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-3">
                          <span className="text-base font-black text-slate-800 dark:text-slate-200 tracking-tight group-hover:text-[#0D95F0] transition-colors">
                            {product.name}
                          </span>
                          {product.is_featured && (
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[10px] font-mono text-slate-400 dark:text-slate-300 uppercase tracking-wider">
                            /{product.slug}
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="py-6 px-8 text-right">
                      <div className="flex items-center justify-end gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                        <Link 
                          ...
                          target="_blank"
                          className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-white dark:bg-slate-900 hover:text-[#0D95F0] hover:shadow-md border border-slate-200/50 flex items-center justify-center text-slate-500 dark:text-slate-400 dark:text-slate-300 transition-all hover:scale-105"
                        ...
                        <Link 
                          ...
                          className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-white dark:bg-slate-900 hover:text-emerald-500 hover:shadow-md border border-slate-200/50 flex items-center justify-center text-slate-500 dark:text-slate-400 dark:text-slate-300 transition-all hover:scale-105"
                          title="Edit Product"
                        >
                          <Edit2 size={15} />
                        </Link>
                        <button 
                          ...
                          className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-white dark:bg-slate-900 hover:text-rose-500 hover:shadow-md border border-slate-200/50 flex items-center justify-center text-slate-500 dark:text-slate-400 dark:text-slate-300 transition-all hover:scale-105"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-[#0A1628] dark:text-white tracking-tight">Delete Product</h3>
                <p className="text-slate-400 dark:text-slate-300 text-xs font-bold leading-relaxed">
                  Are you sure you want to permanently delete <strong className="text-slate-800 dark:text-slate-200">"{deleteTarget.name}"</strong>? This action cannot be undone.
                </p>
              </div>
