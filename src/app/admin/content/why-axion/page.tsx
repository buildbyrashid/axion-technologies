...
import { Plus, Edit2, Trash2, Loader2, Sparkles, Cpu, Zap, Workflow } from 'lucide-react'
...
export default function WhyAxionPage() {
  ...
  async function fetchItems() {
    setLoading(true)
    try {
      const res = await fetch('/api/admin/why-axion')
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const res = await fetch(
      isEdit ? `/api/admin/why-axion/${editingItem!.id}` : '/api/admin/why-axion',
      { method: isEdit ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }
    )
    const json = await res.json()
    if (json.success) { toast.success('Benefit Protocol Synchronized'); fetchItems(); setIsDrawerOpen(false) }
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/why-axion/${id}`, { method: 'DELETE' })
    const json = await res.json()
    if (json.success) { setItems(items.filter(i => i.id !== id)); toast.success('Benefit Removed') }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#0D95F0]/10 flex items-center justify-center text-[#0D95F0]"><Workflow size={20} /></div>
        ...
        <button onClick={() => { setEditingItem(null); setIsDrawerOpen(true) }}
        ...
        ><Plus size={20} /> Append Benefit</button>
      </div>

      <div className="space-y-6">
        <AnimatePresence mode="popLayout">
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-32 rounded-[1.5rem] bg-slate-50 animate-pulse border border-black/5" />)
          ) : items.length === 0 ? (
            ...
            <motion.div key={item.id} layout initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: index * 0.1 }}
            ...
            >
              ...
              <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                <button onClick={() => { setEditingItem(item); setIsDrawerOpen(true) }}
              ...
              <div className="absolute top-0 right-0 p-12 opacity-[0.03] group-hover:opacity-10 transition-opacity pointer-events-none"><Sparkles size={160} /></div>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>

      <SpatialDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)}
      ...
      >
        <form onSubmit={handleSave} className="space-y-8 py-6">
          <div className="space-y-2">
            ...
            <input name="title" defaultValue={editingItem?.title} required placeholder="e.g. Advanced Holographic Optics"
          ...
          <div className="space-y-2">
            ...
            <input name="icon_name" defaultValue={editingItem?.icon_name} placeholder="e.g. Zap, Shield, Award"
              className="w-full px-6 py-4 rounded-2xl border border-black/5 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none text-sm font-bold tracking-tight transition-all"
            />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Proposition Context</label>
            <textarea name="description" defaultValue={editingItem?.description} required rows={6}
