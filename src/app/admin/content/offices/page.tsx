...
import { Plus, MapPin, Globe2, Edit2, Trash2, Loader2, Building2, Phone, Mail, ArrowUpRight, Info } from 'lucide-react'
...
export default function OfficesPage() {
  ...
  async function fetchOffices() {
    ...
    try {
      ...
      else toast.error('Failed to load offices')
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      role,
      is_headquarters: role === 'HQ',
    }
    const isEdit = !!editingOffice?.id
    const res = await fetch(
      isEdit ? `/api/admin/offices/${editingOffice!.id}` : '/api/admin/offices',
      { method: isEdit ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }
    )
    const json = await res.json()
    if (json.success) { toast.success('Office saved successfully'); fetchOffices(); setIsDrawerOpen(false); setIsDirty(false) }
    else toast.error('Save failed: ' + (json.error || 'Unknown'))
    setSaving(false)
  }

  async function handleDelete(id: string) {
    if (!confirm('Archive this location from the global map?')) return
    const res = await fetch(`/api/admin/offices/${id}`, { method: 'DELETE' })
    const json = await res.json()
    if (json.success) { setOffices(offices.filter(o => o.id !== id)); toast.success('Location archived') }
    else toast.error('Delete failed')
  }

  const handleDiscard = () => { setIsDirty(false); setIsDrawerOpen(false) }
  ...
  return (
    <div className="space-y-12 pb-24">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#0D95F0]/10 flex items-center justify-center text-[#0D95F0]"><Globe2 size={20} /></div>
        ...
        <button onClick={() => { setEditingOffice(null); setIsDrawerOpen(true); setIsDirty(false) }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-6 min-h-[600px]">
        {loading ? (
          ...
          <div className="col-span-full py-32 text-center bg-white/40 backdrop-blur-xl rounded-[1.75rem] border border-black/5 border-dashed">
            <div className="w-24 h-24 rounded-[1.5rem] bg-slate-50 flex items-center justify-center text-slate-200 mx-auto mb-8"><MapPin size={48} /></div>
            ...
            <button onClick={() => setIsDrawerOpen(true)} className="px-8 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-[#0A1628] hover:bg-slate-50 transition-all">Deploy First Office</button>
          </div>
        ) : (
          offices.map((office, index) => {
            const isLarge = index === 0 || index === 5
            return (
              <motion.div key={office.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.05 }}
                className={cn("group relative overflow-hidden bg-white rounded-[1.5rem] border border-black/5 p-10 transition-all duration-700 hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] hover:-translate-y-2", isLarge ? "md:col-span-3" : "md:col-span-2")}
              >
                ...
                <div className="relative z-10 h-full flex flex-col">
                  <div className="flex items-start justify-between mb-8">
                    <div className={cn("w-16 h-16 rounded-[2rem] flex items-center justify-center transition-all duration-700 group-hover:scale-110 group-hover:rotate-6", office.role === 'HQ' ? "bg-slate-950 text-white shadow-2xl shadow-black/20" : "bg-white border border-black/5 text-slate-400")}>
                      <Building2 size={28} />
                    </div>
                    <SpatialBadge variant={office.role === 'HQ' ? 'emerald' : office.role === 'R&D' ? 'amber' : 'blue'} pulse={office.role === 'HQ'}>
                  ...
                  <p className="text-slate-500 font-medium leading-relaxed mb-10 line-clamp-3">{office.address}</p>
                  <div className="mt-auto space-y-4">
                    <div className="flex flex-wrap gap-4">
                      <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-2xl text-[11px] font-bold text-slate-600 border border-black/5">
                        <Mail size={14} className="text-[#0D95F0]" />{office.email}
                      </div>
                      <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-2xl text-[11px] font-bold text-slate-600 border border-black/5">
                        <Phone size={14} className="text-[#0D95F0]" />{office.phone}
                      </div>
                    </div>
                  </div>
                  <div className="absolute top-8 right-8 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
                    <button onClick={() => { setEditingOffice(office); setIsDrawerOpen(true); setIsDirty(false) }}
                  ...
                  <div className="absolute bottom-8 right-8 text-slate-200 group-hover:text-[#0D95F0]/20 transition-colors duration-700"><ArrowUpRight size={64} /></div>
                </div>
              </motion.div>
            )
          })
      ...
      <SpatialDrawer isOpen={isDrawerOpen} onClose={handleDiscard} title={editingOffice ? 'Edit Location' : 'Deploy New Location'} description="Configure technical and logistical details for this office.">
        <form ref={formRef} onChange={() => setIsDirty(true)} className="space-y-10 py-4">
          <div className="space-y-8">
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">City</label>
                <input name="city" defaultValue={(editingOffice as any)?.city} required placeholder="e.g. Dubai"
              ...
              <div className="space-y-2">
                ...
                <input name="country" defaultValue={(editingOffice as any)?.country} required placeholder="e.g. UAE"
                  className="w-full px-6 py-4 rounded-[1.5rem] border border-black/5 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none text-sm font-bold transition-all"
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <textarea name="address" defaultValue={editingOffice?.address} required rows={3} placeholder="Full technical address..."
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input name="email" type="email" defaultValue={editingOffice?.email} required placeholder="contact@axion.com"
              ...
              <div className="space-y-2">
                ...
                <input name="phone" defaultValue={editingOffice?.phone} required placeholder="+971 ..."
                  className="w-full px-6 py-4 rounded-[1.5rem] border border-black/5 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none text-sm font-medium transition-all"
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <select name="role" defaultValue={(editingOffice as any)?.role || 'Office'}
                className="w-full px-6 py-4 rounded-[1.5rem] border border-black/5 bg-slate-50 focus:bg-white outline-none text-sm font-bold transition-all appearance-none cursor-pointer"
              >
          ...
          <div className="p-6 bg-slate-50 rounded-[2rem] border border-black/5 flex gap-4">
            <div className="w-10 h-10 rounded-xl bg-white border border-black/5 flex items-center justify-center text-[#0D95F0] shrink-0"><Info size={18} /></div>
            <p className="text-[11px] text-slate-400 font-bold leading-relaxed uppercase tracking-wider">This location will be instantly visible on the global network map upon successful deployment.</p>
          </div>
        </form>
      </SpatialDrawer>

      <SpatialFloatingBar isVisible={isDirty} onSave={handleSave} onDiscard={handleDiscard} isLoading={saving} />
    </div>
  )
}
