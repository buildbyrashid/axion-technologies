...
import { Plus, Edit2, Trash2, Loader2, Building2, Target, Zap, Shield, Activity, Cpu } from 'lucide-react'
...
export default function IndustriesPage() {
  ...
  async function fetchIndustries() {
    ...
    try {
      ...
      else toast.error('Failed to load industries')
    } catch {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const isEdit = !!editingIndustry?.id
    const res = await fetch(
      isEdit ? `/api/admin/industries/${editingIndustry!.id}` : '/api/admin/industries',
  ...
  async function handleDelete(id: string) {
    ...
    const json = await res.json()
    if (json.success) { setIndustries(industries.filter(i => i.id !== id)); toast.success('Sector Removed') }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#0D95F0]/10 flex items-center justify-center text-[#0D95F0]"><Target size={20} /></div>
        ...
        <button onClick={() => { setEditingIndustry(null); setIsDrawerOpen(true) }}
        ...
        >
          <Plus size={20} /> Register New Sector
        </button>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            ...
            industries.map((industry, index) => (
              <motion.div key={industry.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: index * 0.1 }}
              ...
              >
                <div className="flex items-start justify-between mb-8">
                  <div className="w-16 h-16 rounded-[1.5rem] bg-slate-950 text-white flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-xl">
                    <Cpu size={28} />
                  </div>
                ...
                <div className="pt-8 border-t border-black/5 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => { setEditingIndustry(industry); setIsDrawerOpen(true) }}
                    ...
                    <button onClick={() => handleDelete(industry.id)}
      ...
      >
        <form onSubmit={handleSave} className="space-y-8 py-6">
          <div className="space-y-2">
            ...
            <input name="name" defaultValue={editingIndustry?.name} required placeholder="e.g. Aerospace & Defense"
          ...
          <div className="space-y-2">
            ...
            <input name="slug" defaultValue={editingIndustry?.slug} required
          ...
          <div className="space-y-2">
            ...
            <input name="icon_name" defaultValue={editingIndustry?.icon_name} placeholder="e.g. Shield, Zap, Target"
              className="w-full px-6 py-4 rounded-2xl border border-black/5 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none text-sm font-bold tracking-tight transition-all"
            />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Mission Description</label>
            <textarea name="description" defaultValue={editingIndustry?.description} required rows={5}
