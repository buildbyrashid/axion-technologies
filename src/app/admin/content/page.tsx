...
export default function HomepageSettingsPage() {
  ...
  async function fetchData() {
    ...
    if (error) toast.error('Failed to load portal configuration')
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    toast.success('Main Portal Synchronized')
    setSaving(false)
  }
                   ...
                   <div className="space-y-3">
                      ...
                      <div className="flex p-1.5 bg-slate-100 rounded-[1.25rem]">
                        <button
                          ...
                          className={cn(
                            "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-[0.8rem] text-[10px] font-black uppercase tracking-widest transition-all",
                            data.hero_media_type === 'video' ? "bg-white text-[#0A1628] shadow-lg shadow-black/5" : "text-slate-400 hover:text-slate-600"
                          )}
                        >
                          <Video size={14} />
                          Video
                        ...
                        <button
                          ...
                          className={cn(
                            "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-[0.8rem] text-[10px] font-black uppercase tracking-widest transition-all",
                            data.hero_media_type === 'image' ? "bg-white text-[#0A1628] shadow-lg shadow-black/5" : "text-slate-400 hover:text-slate-600"
                          )}
                        >
                          <ImageIcon size={14} />
                          Image
           ...
           <div className="relative aspect-[4/5] rounded-[4rem] overflow-hidden bg-slate-950 shadow-2xl border border-black/10 group">
              ...
              <div className="absolute inset-0 z-0 opacity-40 group-hover:scale-105 transition-transform duration-1000">
                 {data.hero_media_type === 'video' ? (
                    <div className="w-full h-full bg-[#0D95F0]/20 flex items-center justify-center text-[#0D95F0] italic">
                       [ Cinematic Video Sequence Active ]
                    </div>
                 ) : (
                    <div className="w-full h-full bg-slate-800 flex items-center justify-center text-slate-600 italic">
                       [ High-Fidelity Image Layer ]
                    </div>
                 )}
              </div>
              ...
              <div className="absolute inset-0 z-20 p-12 flex flex-col justify-end">
                 ...
                 >
                    ...
                    <p className="text-white/60 text-lg font-medium leading-relaxed max-w-sm">
                       {data.hero_subtext || 'Configuring universal engineering excellence for global visual systems.'}
                    </p>
              ...
              <div className="absolute top-8 left-1/2 -translate-x-1/2 w-32 h-6 bg-black/40 backdrop-blur-md rounded-full z-30 border border-white/10 flex items-center justify-center">
