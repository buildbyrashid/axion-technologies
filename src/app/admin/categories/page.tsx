    ...
    <div className="space-y-12 pb-24 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-2">
          ...
          <h1 className="text-5xl font-extrabold text-[#0A1628] dark:text-white tracking-tighter">Product Categories</h1>
          <p className="text-slate-500 dark:text-slate-400 dark:text-slate-300 text-lg font-medium max-w-xl">Manage the categories and sub-categories that will display on the website navigation and products catalog.</p>
        </div>
        <button 
      ...
      <div className="flex flex-col md:flex-row items-center gap-6">
        <div className="relative flex-1 w-full group">
          ...
          <input 
            ...
            className="relative w-full pl-16 pr-8 py-6 rounded-[2rem] border border-black/5 dark:border-white/10 bg-white dark:bg-slate-900 focus:ring-8 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none transition-all text-base font-bold tracking-tight shadow-sm"
          />
        </div>
        <div className="flex items-center gap-3 px-8 py-4 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-[1.5rem] shadow-sm shrink-0">
          <Activity size={18} className="text-[#0D95F0]" />
          <span className="text-xs font-black text-slate-400 dark:text-slate-300 uppercase tracking-widest">{categories.length} Total Categories</span>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          <div className="py-32 flex flex-col items-center gap-8 bg-white dark:bg-slate-900 rounded-[1.75rem] border border-black/5 dark:border-white/10 border-dashed opacity-40">
            <Layers size={64} className="text-slate-300" />
            <p className="text-slate-500 dark:text-slate-400 dark:text-slate-300 font-bold tracking-tight text-xl">Zero structural layers detected.</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {mainCategories.map((mainCat) => {
              ...
              return (
                <div 
                  key={mainCat.id}
                  className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl hover:shadow-[#0D95F0]/2 transition-all duration-500"
                >
                  {/* Category Header Row */}
                  <div className="p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100">
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <button 
                        onClick={() => toggleExpand(mainCat.id)}
                        className="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 flex items-center justify-center text-slate-500 dark:text-slate-400 dark:text-slate-300 hover:text-slate-800 dark:text-slate-200 transition-all shrink-0"
                      >
                      ...
                      <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-3">
                          <h3 className="text-xl font-black text-[#0A1628] dark:text-white tracking-tighter truncate">{mainCat.name}</h3>
                          <span className="text-[10px] font-black bg-[#0D95F0]/10 text-[#0D95F0] px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                            Main
                          </span>
                        </div>
                        <p className="text-[11px] font-black text-slate-400 dark:text-slate-300 uppercase tracking-[0.2em] italic truncate">/{mainCat.slug}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 shrink-0 sm:self-center">
                      <button
                        ...
                        className="h-10 px-4 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-800 text-[#0D95F0] border border-[#0D95F0]/20 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all"
                      >
                        <Plus size={14} />
                        Add Sub-Category
                      </button>
                      ...
                      <div className="flex items-center gap-2">
                        <button 
                          ...
                          className="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 text-[#0D95F0] hover:bg-white dark:bg-slate-900 hover:shadow-lg flex items-center justify-center transition-all hover:scale-105"
                        ...
                        <button 
                          ...
                          className="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 text-rose-500 hover:bg-white dark:bg-slate-900 hover:shadow-lg flex items-center justify-center transition-all hover:scale-105"
                  ...
                  {isExpanded && (
                    <div className="p-6 bg-white dark:bg-slate-900 space-y-4">
                      {mainSubs.length === 0 ? (
                        <div className="py-8 text-center text-slate-400 dark:text-slate-300 text-xs font-bold italic">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {mainSubs.map((sub) => (
                            <div 
                              key={sub.id}
                              className="p-5 border border-slate-100 rounded-xl bg-slate-50/30 dark:bg-slate-800/30 hover:bg-slate-50 dark:bg-slate-800 transition-colors flex items-center justify-between gap-4 group"
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-300 flex items-center justify-center font-bold text-xs shrink-0">
                                  L2
                                </div>
                                <div className="flex flex-col min-w-0">
                                  <h4 className="text-sm font-black text-slate-700 truncate tracking-tight">{sub.name}</h4>
                                  <span className="text-[10px] font-black text-slate-400 dark:text-slate-300 uppercase tracking-widest truncate">/{sub.slug}</span>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-3 shrink-0">
                                ...
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                  <button 
                                    ...
                                    className="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200/60 text-[#0D95F0] flex items-center justify-center hover:scale-110 active:scale-95 transition-all shadow-sm"
                                  ...
                                  <button 
                                    ...
                                    className="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200/60 text-rose-500 flex items-center justify-center hover:scale-110 active:scale-95 transition-all shadow-sm"
        ...
        <form onSubmit={handleSaveCategory} className="space-y-8 py-6">
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              required
              className="w-full px-6 py-5 rounded-2xl border border-black/5 dark:border-white/10 bg-slate-50 dark:bg-slate-800 focus:bg-white dark:bg-slate-900 focus:ring-4 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none transition-all text-sm font-bold tracking-tight"
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 dark:text-slate-300 uppercase tracking-widest ml-1">Meta Description</label>
            <textarea 
              name="description" 
              defaultValue={editingCategory?.description || ''} 
              rows={5}
              className="w-full px-6 py-5 rounded-2xl border border-black/5 dark:border-white/10 bg-slate-50 dark:bg-slate-800 focus:bg-white dark:bg-slate-900 focus:ring-4 focus:ring-[#0D95F0]/5 focus:border-[#0D95F0]/20 outline-none transition-all text-sm font-bold tracking-tight resize-none leading-relaxed"
        ...
        {deleteTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-[#0A1628] dark:text-white tracking-tight">Delete Category</h3>
                <p className="text-slate-400 dark:text-slate-300 text-xs font-bold leading-relaxed">
                  Are you sure you want to delete <strong className="text-slate-800 dark:text-slate-200">"{deleteTarget.name}"</strong>? This will permanently remove it from the database and site navigation.
                </p>
              </div>
